...
public final class DlgRujuk extends javax.swing.JDialog {
    ...
    private String diagnosa="",diagnosa2="",keluar="",status="",tgl="",sql="",pekerjaan="",finger="",jaminan="",nopeserta="";
...
//        TTmpRujuk.setDocument(new batasInput((byte)45).getKata(TTmpRujuk));
    ...
    private void initComponents() {
        ...
        TTmpRujuk = new javax.swing.JTextArea();
        ...
        TDiagnosa.setBounds(740, 70, 282, 23);

        jLabel9.setText("Diagnosa :");
        jLabel9.setName("jLabel9"); // NOI18N
        FormInput.add(jLabel9);
        jLabel9.setBounds(680, 70, 60, 23);
        ...
        DTPRujuk.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "04-04-2023" }));
        ...
        jLabel15.setBounds(400, 70, 90, 23);
        ...
        btnDiagnosa.setBounds(1030, 70, 28, 23);

        jScrollPane1.setName("jScrollPane1"); // NOI18N

        TTmpRujuk.setColumns(20);
        TTmpRujuk.setRows(5);
        TTmpRujuk.setName("TTmpRujuk"); // NOI18N
        jScrollPane1.setViewportView(TTmpRujuk);

        FormInput.add(jScrollPane1);
        jScrollPane1.setBounds(80, 70, 290, 50);
...
//                    "',permintaan='"+Permintaan.getSelectedItem().toString()+"'");
...
private void TDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_TDokterKeyPressed
    ...
    private void MnSuratRujukanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnSuratRujukanActionPerformed
        if(TPasien.getText().trim().equals("")){
            ...
            keluar=Sequel.cariIsi("select stts_pulang from kamar_inap where no_rawat=? and stts_pulang='-' order by STR_TO_DATE(concat(tgl_masuk,' ',jam_masuk),'%d-%m-%y %H:%i:%s') desc limit 1",TNoRw.getText());

            param.put("html","dan berdasarkan riwayat penyakit / kasus serta hasil pemeriksaan , kami simpulkan dengan diagnosis ; ");
            param.put("htmldiag",tbObat.getValueAt(tbObat.getSelectedRow(),7).toString());
            param.put("html2","Demikian kami sampaikan, atas perhatian dan kerja-sama yang diberikan diucapkan terima kasih");
            param.put("namars",akses.getnamars());
            param.put("alamatrs",akses.getalamatrs());
            ...
            param.put("emailrs",akses.getemailrs());
//            param.put("logo",Sequel.cariGambar("select setting.logo from setting"));
            param.put("spesialis",Sequel.cariIsi("Select spesialis.nm_sps from spesialis inner join dokter on dokter.kd_sps=spesialis.kd_sps inner join rujuk on rujuk.kd_dokter=dokter.kd_dokter where rujuk.no_rawat=?",TNoRw.getText()));
            param.put("jaminan",Sequel.cariIsi("select png_jawab from penjab  inner join reg_periksa on reg_periksa.kd_pj=penjab.kd_pj  where reg_periksa.no_rawat=?",TNoRw.getText()));
            param.put("nopeserta",Sequel.cariIsi("select no_peserta from pasien inner join reg_periksa on reg_periksa.no_rkm_medis=pasien.no_rkm_medis where reg_periksa.no_rawat=?",TNoRw.getText()));
            param.put("pekerjaan",Sequel.cariIsi("select pekerjaan from pasien inner join reg_periksa on reg_periksa.no_rkm_medis=pasien.no_rkm_medis where reg_periksa.no_rawat=?",TNoRw.getText()));
            param.put("logo",Sequel.cariGambar("select setting.logo from setting"));
            ...
            param.put("finger","Surat atas nama "+TPasien.getText()+" dengan No.Surat:"+TNoRj.getText()+"/445/RSUD.UM.PP Dikeluarkan di "+akses.getnamars()+", Kabupaten/Kota "+akses.getkabupatenrs()+"\nDitandatangani secara elektronik oleh "+TDokter.getText()+"\nID "+(finger.equals("")?KdDok.getText():finger)+"\n"+DTPRujuk.getSelectedItem());
    ...
    */
    ...
    private javax.swing.JTextArea TTmpRujuk;
...
//        Valid.autoNomer("rujuk","R",9,TNoRj);
        Valid.autoNomer3("select ifnull(MAX(CONVERT(rujuk.no_rujuk,signed)),0) from rujuk where rujuk.tgl_rujuk='"+Valid.SetTgl(DTPRujuk.getSelectedItem()+"")+"' ","",3,TNoRj);
...
//            Spesialis.setText(tbObat.getValueAt(tbObat.getSelectedRow(),10).toString());
           
//            Permintaan.setSelectedItem(tbObat.getValueAt(tbObat.getSelectedRow(),13).toString());
